...
// 1st Error: in the 'moveTortois' and 'moveHare' functions the parameters were not pointers to the memory location.
// 2nd Error: in the 'printCurrentLocation' function the variables were not pointers to the memory location.
// 3rd Error: in the main function in the part that it was calling the 'moveTortois','moveHare' and 'printCurrentLocation' the parameters were not given the memory address(&).
// 4th Error: in the 'moveTortois' and 'moveHare' functions the variables were not pointers to the memory location.
// 5th Error: The pointers 'hPos','tPos','harePos','tortPos' is uninitialized and doesn't point to valid memory.
// Attempting to dereference it leads to undefined behavior and it causes a segmentation fault.
// Fixing the error in the main: Allocate memory for 'hPos','tPos','harePos','tortPos' by assigning it the address of an existing variable.
// Fixing the error in the functions: by assigning pointers for 'hPos','tPos','harePos','tortPos' in the functions 'moveTortois','moveHare' and 'printCurrentLocation'.
